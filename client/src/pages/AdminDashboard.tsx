...
import {
  Upload, LogOut, Trash2, Pencil, Plus, Check, X,
  Image, Music, BookOpen, Megaphone, LayoutDashboard,
} from "lucide-react";
...
function BannerManager() {
  ...
  const createMutation = trpc.bannerSlides.create.useMutation({ onSuccess: () => { utils.bannerSlides.listAll.invalidate(); utils.bannerSlides.list.invalidate(); toast.success("Banner 已新增"); resetForm(); } });
  const deleteMutation = trpc.bannerSlides.delete.useMutation({ onSuccess: () => { utils.bannerSlides.listAll.invalidate(); utils.bannerSlides.list.invalidate(); toast.success("已刪除"); } });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 space-y-3">
        <p className="font-semibold text-blue-900">新增 Banner</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div><Label>外部連結（選填）</Label><Input className="mt-1" value={link} onChange={e => setLink(e.target.value)} placeholder="https://..." /></div>
...
function VideoManager() {
  ...
  const createMutation = trpc.videos.create.useMutation({ onSuccess: () => { utils.videos.listAll.invalidate(); utils.videos.list.invalidate(); toast.success("影片已新增"); resetForm(); } });
  const deleteMutation = trpc.videos.delete.useMutation({ onSuccess: () => { utils.videos.listAll.invalidate(); utils.videos.list.invalidate(); toast.success("已刪除"); } });
  const updateMutation = trpc.videos.update.useMutation({ onSuccess: () => { utils.videos.listAll.invalidate(); utils.videos.list.invalidate(); toast.success("已更新"); setEditId(null); } });
  ...
  const resetForm = () => { setTitle(""); setVideoUrl(""); setDescription(""); setLanguage("閩南語"); setCategory("一般"); };

  const handleCreate = () => {
    if (!title.trim() || !videoUrl.trim()) { toast.error("請填寫標題和影片連結"); return; }
    createMutation.mutate({ title, videoUrl, description: description || undefined, language, category });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div><Label>語言</Label>
            <select className="mt-1 w-full h-10 px-3 rounded-md border text-sm" value={language} onChange={e => setLanguage(e.target.value)}>
        ...
        <Button onClick={handleCreate} disabled={createMutation.isPending} className="bg-green-600 hover:bg-green-700 text-white"><Plus className="w-4 h-4 mr-2" />新增影片</Button>
      </div>
      <div className="space-y-3">
        ...
        {videos?.map((v) => (
          <div key={v.id} className="border rounded-lg p-4 bg-white space-y-2">
            {editId === v.id ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="bg-green-600 text-white" onClick={() => updateMutation.mutate({ id: v.id, title: editTitle, videoUrl: editUrl, description: editDesc })}><Check className="w-4 h-4 mr-1" />儲存</Button>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" onClick={() => { setEditId(v.id); setEditTitle(v.title); setEditUrl(v.videoUrl); setEditDesc(v.description ?? ""); }}><Pencil className="w-4 h-4" /></Button>
...
function PhotoManager() {
  ...
  const createMutation = trpc.photos.create.useMutation({ onSuccess: () => { utils.photos.listAll.invalidate(); utils.photos.list.invalidate(); toast.success("照片已新增"); resetForm(); } });
  const deleteMutation = trpc.photos.delete.useMutation({ onSuccess: () => { utils.photos.listAll.invalidate(); utils.photos.list.invalidate(); toast.success("已刪除"); } });
  ...
  const resetForm = () => { setTitle(""); setAlbum("母語日活動"); setYear("113"); };
    ...
    try {
      ...
      reader.onload = async (ev) => {
        try {
          ...
          await createMutation.mutateAsync({ title, imageUrl: result.url, imageKey: result.key, albumName: album, year });
...
function PlanManager() {
  ...
  const createMutation = trpc.plans.create.useMutation({ onSuccess: () => { utils.plans.listAll.invalidate(); toast.success("計畫已新增"); resetForm(); } });
  const deleteMutation = trpc.plans.delete.useMutation({ onSuccess: () => { utils.plans.listAll.invalidate(); toast.success("已刪除"); } });
  ...
  const TYPE_LABELS: Record<string, string> = {
    mother_tongue_day: "母語日活動",
    curriculum_plan: "課程計畫",
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div><Label>類型</Label>
            <select className="mt-1 w-full h-10 px-3 rounded-md border text-sm" value={type} onChange={e => setType(e.target.value as any)}>
              <option value="curriculum_plan">課程計畫</option>
              <option value="mother_tongue_day">母語日活動</option>
              <option value="teaching_material">自編教材</option>
        ...
        {plans?.map((p) => (
          <div key={p.id} className="flex items-start gap-3 border rounded-lg p-4 bg-white">
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">{TYPE_LABELS[p.type]} · {p.year}學年度</p>
              {p.externalUrl && <a href={p.externalUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline">開啟連結</a>}
            </div>
...
function AnnouncementManager() {
  ...
  const createMutation = trpc.announcements.create.useMutation({ onSuccess: () => { utils.announcements.listAll.invalidate(); utils.announcements.list.invalidate(); toast.success("公告已新增"); setTitle(""); setContent(""); } });
  const deleteMutation = trpc.announcements.delete.useMutation({ onSuccess: () => { utils.announcements.listAll.invalidate(); utils.announcements.list.invalidate(); toast.success("已刪除"); } });
  const updateMutation = trpc.announcements.update.useMutation({ onSuccess: () => { utils.announcements.listAll.invalidate(); utils.announcements.list.invalidate(); toast.success("已更新"); setEditId(null); } });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 space-y-3">
        ...
        <Button onClick={() => { if (!title.trim()) { toast.error("請填寫標題"); return; } createMutation.mutate({ title, content }); }} disabled={createMutation.isPending} className="bg-green-600 hover:bg-green-700 text-white"><Plus className="w-4 h-4 mr-2" />新增公告</Button>
      </div>
      <div className="space-y-3">
          ...
          <div key={a.id} className="border rounded-lg p-4 bg-white">
            {editId === a.id ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="bg-green-600 text-white" onClick={() => updateMutation.mutate({ id: a.id, title: editTitle, content: editContent })}><Check className="w-4 h-4 mr-1" />儲存</Button>
                  <Button size="sm" variant="outline" onClick={() => setEditId(null)}><X className="w-4 h-4 mr-1" />取消</Button>
                </div>
              </div>
            ) : (
              <div className="flex items-start gap-3">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" onClick={() => { setEditId(a.id); setEditTitle(a.title); setEditContent(a.content ?? ""); }}><Pencil className="w-4 h-4" /></Button>
...
export default function AdminDashboard() {
  ...
  const logoutMutation = trpc.auth.logout.useMutation({ onSuccess: () => { localStorage.removeItem("admin_password"); window.location.href = "/login"; } });
